...
# این اسکریپت فایل App.js پروژه را با نسخه‌ی جدید جایگزین می‌کند و دو مشکل را رفع می‌کند:
#   ۱) نوتیفیکیشن دائمی از این به بعد فقط عادت‌های «امروز» را می‌نویسد
#      (عادت‌های روزانه + بخش‌هایی که امروز جزو روزهایشان است) — نه همه‌ی عادت‌های برنامه.
#      محتوای این نوتیف با هر بار باز شدن برنامه هم با تاریخ روز از نو حساب می‌شود.
#   ۲) دکمه‌ی «تایمر عادت رندوم» دیگر برنامه را باز نگه نمی‌دارد؛ اگر اندروید
#      برنامه را باز کند، بلافاصله به پس‌زمینه برگردانده می‌شود.
...
#   bash apply_today_only_notif_fix.sh
...
const RANDOM_TIMER_TICK_SECONDS = 5;
...
async function configureNotificationChannel() {
  ...
  // کانال جدا با اهمیت پایین برای شمارش معکوس زنده‌ی تایمر رندوم — هر ثانیه
  // آپدیت می‌شود، پس باید کاملاً بی‌صدا و بدون هشدار (heads-up) باشد؛ فقط
  // در لحظه‌ی پایان از کانال TIMER_CHANNEL_ID (فوری/پرصدا) استفاده می‌کنیم.
  await Notifications.setNotificationChannelAsync(RANDOM_TIMER_LIVE_CHANNEL_ID, {
    name: 'شمارش معکوس زنده‌ی تایمر رندوم',
...
// عادت‌های هنوز انجام‌نشده‌ی امروز به‌صورت رندوم انتخاب می‌شود و ۳۰ ثانیه‌ی
// بعد یک نوتیفیکیشن فوری (urgent) اعلام می‌کند که وقتش تمام شده.
// شناسه‌ی interval فعلیِ شمارش معکوس زنده — در سطح ماژول نگه داشته می‌شود
// (نه داخل یک کامپوننت)، چون این تایمر باید مستقل از این‌که کاربر توی کدوم
// صفحه‌ی برنامه‌ست ادامه پیدا کنه، و فقط یک تایمر رندوم هم‌زمان معنی داره.
let randomTimerIntervalHandle = null;

async function updateRandomTimerLiveNotification(habitTitle, secondsLeft) {
  try {
    await Notifications.scheduleNotificationAsync({
      identifier: RANDOM_TIMER_LIVE_ID,
      content: {
        title: '⏱️ تایمر عادت رندوم',
        body: `${habitTitle} — ${toPersianDigits(secondsLeft)} ثانیه مانده`,
        // عمداً sticky/autoDismiss نداره — کاربر هر وقت خواست خودش می‌تونه
        // این نوتیف رو با لمس/کشیدن پاک کنه.
        sound: false,
        data: { kind: 'tick' },
    ...
    console.warn('Failed to update live random timer notification', e);
  }
}

// از همون identifier نوتیف در حال شمارش استفاده می‌کنیم — یعنی وقتی این
// نوتیف پایان (روی کانال urgent) ارسال بشه، به‌جای ساختن یه نوتیف جدید،
// نوتیف در-حال-شمارش رو جایگزین می‌کنه. این یعنی حتی اگه جاوااسکریپت کامل
// مرده باشه و کد پاک‌سازی اجرا نشه، وقتی سیستم‌عامل خودش این نوتیف تضمینی
// رو (با همون شناسه) نشون بده، نوتیف گیرکرده‌ی قبلی خودبه‌خود جایگزین/حذف
// می‌شه — بدون نیاز به این‌که هیچ کد جاوااسکریپتی در آن لحظه اجرا شود.
async function finishRandomHabitTimer(habitTitle) {
  try {
    await Notifications.scheduleNotificationAsync({
      identifier: RANDOM_TIMER_LIVE_ID,
      content: {
        title: '⏱️ پایان تایمر عادت',
        body: `تایمر ${habitTitle} تموم شد، برو انجامش بده`,
        sound: true,
        priority: Notifications.AndroidNotificationPriority.MAX,
        data: { kind: 'finish' },
    ...
    console.warn('Failed to send timer-finished notification', e);
  }
}
...
async function scheduleRandomHabitTimer(habits, categories) {
  ...
  // اگر یک شمارش معکوس رندوم دیگر از قبل در حال اجراست، متوقفش کن و از نو
  // (با عادت تازه‌ی انتخاب‌شده) شروع کن — تا دو interval هم‌زمان اجرا نشود.
  if (randomTimerIntervalHandle) {
    clearInterval(randomTimerIntervalHandle);
    randomTimerIntervalHandle = null;
  }
  try {
    await Notifications.cancelScheduledNotificationAsync(RANDOM_TIMER_LIVE_ID);
  } catch (e) {}

  // تضمینِ سطح سیستم‌عامل: این نوتیف را با trigger واقعی (نه setTimeout جاوااسکریپت)
  // زمان‌بندی می‌کنیم، پس دقیقاً بعد از ۳۰ ثانیه می‌رسد — حتی اگر در همین حین
  // پردازه‌ی جاوااسکریپت به‌خاطر محدودیت‌های پس‌زمینه‌ی گوشی متوقف/کشته شود و
  // شمارش معکوس زنده‌ی پایین نتواند خودش را کامل کند. از همان identifier
  // نوتیف زنده استفاده می‌کنیم تا وقتی این فال‌بک برسد، نوتیف در-حال-شمارشِ
  // احتمالاً گیرکرده را خودبه‌خود (توسط خودِ اندروید، بدون نیاز به اجرای
  // جاوااسکریپت) جایگزین کند.
  try {
    await Notifications.scheduleNotificationAsync({
      identifier: RANDOM_TIMER_LIVE_ID,
      content: {
    ...
    console.warn('Failed to schedule guaranteed timer-finished notification', e);
  }

  // شمارش معکوس زنده روی نوتیف — تلاش بهترین‌حالت (best effort)؛ هر ۵ ثانیه
  // آپدیت می‌شود. اگر این interval به هر دلیلی نتواند ادامه پیدا کند،
  // نوتیف تضمینی بالا در هر صورت سر وقت (۳۰ ثانیه) می‌رسد و جایگزینش می‌شود.
  let secondsLeft = RANDOM_TIMER_SECONDS;
  await updateRandomTimerLiveNotification(randomHabit.title, secondsLeft);

  randomTimerIntervalHandle = setInterval(() => {
    secondsLeft -= RANDOM_TIMER_TICK_SECONDS;
    if (secondsLeft <= 0) {
      clearInterval(randomTimerIntervalHandle);
      randomTimerIntervalHandle = null;
      // چون خودمان تا انتها زنده ماندیم، نسخه‌ی فال‌بک زمان‌بندی‌شده را کنسل
      // می‌کنیم و بلافاصله خودمان اعلام پایان می‌دهیم — تا دو نوتیف پایان
      // پشت سر هم نیاید.
      Notifications.cancelScheduledNotificationAsync(RANDOM_TIMER_LIVE_ID).catch(() => {});
      finishRandomHabitTimer(randomHabit.title);
    } else {
      updateRandomTimerLiveNotification(randomHabit.title, secondsLeft);
    }
  }, RANDOM_TIMER_TICK_SECONDS * 1000);
}
...
function RootApp() {
  ...
  // اگر نوتیف فال‌بک تضمینی (پایان تایمر) واقعاً برسد — یعنی interval زنده‌ی
  // شمارش معکوس نتوانسته خودش پایان را اعلام کند — نوتیف «در حال شمارش» را
  // پاک می‌کنیم تا برای همیشه روی عدد قدیمی گیر نکند. این فقط زمانی اجرا
  // می‌شود که جاوااسکریپت هنوز زنده باشد؛ در غیر این صورت محدودیت پلتفرم است.
  // اگر نوتیف پایان تضمینی واقعاً برسد (چه از طریق خود جاوااسکریپت، چه از
  // طریق فال‌بک زمان‌بندی‌شده‌ی سیستم‌عامل) و در همین حین یک interval زنده
  // هنوز فعال مانده باشد، پاکش می‌کنیم — صرفاً یک پاک‌سازی احتیاطی؛ خودِ
  // نمایش/جایگزینی نوتیف چون از یک identifier مشترک استفاده می‌کند، توسط
  // اندروید انجام می‌شود و نیازی به اجرای این کد ندارد.
  useEffect(() => {
    const sub = Notifications.addNotificationReceivedListener((notification) => {
      if (notification.request.content.data?.kind === 'finish' && randomTimerIntervalHandle) {
        clearInterval(randomTimerIntervalHandle);
        randomTimerIntervalHandle = null;
      }
    });
...
git add .
git commit -m "fix: نوتیف دائمی فقط عادت‌های امروز را نشان می‌دهد + دکمه تایمر رندوم دیگر برنامه را باز نگه نمی‌دارد"
