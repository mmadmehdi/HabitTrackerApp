...
function ReorderableHabitList({ habitIds, habitsById, onReorderEnd }) {
  ...
  const finalizeDrop = useCallback(() => {
    setOrder((prevOrder) => {
      ...
      const to = dropIndex;
      if (from === null || to === null || from === to) return prevOrder;
        ...
        return (
          <View key={id} style={[styles.reorderDragSlot, { top: idx * REORDER_ROW_HEIGHT }]}>
            <DraggableReorderRow
              ...
              isDragged={isDragged}
              dragY={dragY}
              panHandlers={makePanResponder(idx).panHandlers}
            />
          </View>
        );
      })}
