...
# این اسکریپت فایل App.js پروژه را با نسخه‌ی جدید (تیک هر ۵ ثانیه + نوتیف پایان تضمینی) جایگزین می‌کند
...
#   bash apply_guaranteed_timer.sh
...
const RANDOM_TIMER_FALLBACK_FINISH_ID = 'habit_random_timer_fallback_finish';
...
async function finishRandomHabitTimer(habitTitle) {
  try {
    await Notifications.dismissNotificationAsync(RANDOM_TIMER_LIVE_ID);
  } catch (e) {}
  try {
    await Notifications.scheduleNotificationAsync({
      content: {
...
async function scheduleRandomHabitTimer(habits, categories) {
  ...
  try {
    await Notifications.cancelScheduledNotificationAsync(RANDOM_TIMER_FALLBACK_FINISH_ID);
  } catch (e) {}
  ...
  // شمارش معکوس زنده‌ی پایین نتواند خودش را کامل کند.
  try {
    await Notifications.scheduleNotificationAsync({
      identifier: RANDOM_TIMER_FALLBACK_FINISH_ID,
      content: {
        title: '⏱️ پایان تایمر عادت',
  ...
  // نوتیف تضمینی بالا در هر صورت سر وقت (۳۰ ثانیه) می‌رسد.
  ...
  randomTimerIntervalHandle = setInterval(() => {
    ...
    if (secondsLeft <= 0) {
      ...
      // پشت سر هم نیاید.
      Notifications.cancelScheduledNotificationAsync(RANDOM_TIMER_FALLBACK_FINISH_ID).catch(() => {});
...
function RootApp() {
  ...
  useEffect(() => {
    const sub = Notifications.addNotificationReceivedListener((notification) => {
      if (notification.request.identifier === RANDOM_TIMER_FALLBACK_FINISH_ID) {
        if (randomTimerIntervalHandle) {
          clearInterval(randomTimerIntervalHandle);
          randomTimerIntervalHandle = null;
        }
        Notifications.dismissNotificationAsync(RANDOM_TIMER_LIVE_ID).catch(() => {});
      }
    });
    return () => sub.remove();
  }, []);
...
git add .
git commit -m "fix: تیک هر 5 ثانیه (به‌جای 1 ثانیه) + نوتیف پایان تضمینی سطح سیستم‌عامل برای تایمر رندوم"
