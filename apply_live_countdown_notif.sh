...
# این اسکریپت فایل App.js پروژه را با نسخه‌ی جدید (شمارش معکوس زنده روی نوتیفیکیشن) جایگزین می‌کند
...
#   bash apply_live_countdown_notif.sh
...
git add .
git commit -m "feat: شمارش معکوس زنده (هر ثانیه) روی نوتیفیکیشن تایمر عادت رندوم"
